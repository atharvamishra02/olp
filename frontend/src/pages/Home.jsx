        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          ...
          {courses.map(course => (
            <div key={course._id} className="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-shadow p-5 flex flex-col border border-transparent hover:border-blue-200 relative group">
              {course.image && <img src={course.image.startsWith('http') ? course.image : `${API_BASE}${course.image}`} alt={course.title} className="rounded-xl mb-4 w-full h-40 object-cover group-hover:scale-105 transition-transform" />}
